...
with c1:
    farmgate_price = st.number_input("Farmgate Sourcing Price", value=85000, step=5000, help="Purchase price from farmer/mandi per ton")
    processing_pack = st.number_input("Sorting, Washing & Packaging", value=25000, step=2000, help="Cold chain, grading, and export cartons")
with c2:
    inland_freight = st.number_input("Inland Transport to Port", value=15000, step=1000, help="Trucking to Karachi Port / Port Qasim")
    customs_dpp = st.number_input("Customs, DPP Inspection & Certs", value=12000, step=1000, help="Phytosanitary, WeBOC & clearance fees")
with c3:
    ocean_freight = st.number_input("International Freight (Reefer)", value=45000, step=2000, help="Container shipping cost per ton equivalent")
    import_duty_pct = st.number_input("Destination Tariff / Duty (%)", value=0.0, step=0.5, help="Import tariff rate at destination")
